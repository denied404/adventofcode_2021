...
def draw_line(map, line)
  ...
  elsif (line[:x1] - line[:x2]).abs == (line[:y1] - line[:y2]).abs
    xs = if line[:x1] <= line[:x2]
           (line[:x1]..line[:x2]).to_a
         else
           (line[:x2]..line[:x1]).to_a.reverse
         end
    ys = if line[:y1] <= line[:y2]
           (line[:y1]..line[:y2]).to_a
         else
           (line[:y2]..line[:y1]).to_a.reverse
         end
